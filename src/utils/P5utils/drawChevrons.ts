...
const chevronArray: {x: number; z: number; direction: number}[] = [{x: 150, z: -600, direction: Math.PI/2}, {
...
export default function drawChevrons(p: p5) {
    for (const chevron of chevronArray) {
        p.push();
        p.stroke('white');
        p.translate(chevron.x, 100, chevron.z);
        p.rotateY(chevron.direction);
        p.sphere(((p.frameCount%6)/5)+1);
        for (let i = 1; i <=5; i++) {
            p.translate(-5, 0, -5*i);
            p.sphere((((p.frameCount+i)%6)/6)+1);
