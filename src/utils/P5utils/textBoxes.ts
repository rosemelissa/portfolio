import { ITextBox } from "../../interfaces";

export const textBoxes: ITextBox[] = [
  {
    position: { x: 800, y: -80, z: -300, direction: 0 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Todo list",
    textSize: 10,
    plane: {
      width: 150,
      height: 50,
    },
  },
  {
    position: { x: 800, y: -80, z: -100, direction: 0 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Restaurant booking system",
    textSize: 10,
    plane: {
      width: 150,
      height: 50,
    },
  },
  {
    position: { x: 800, y: -80, z: 100, direction: 0 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "YouTube-to-Spotify playlist converter",
    textSize: 10,
    plane: {
      width: 150,
      height: 50,
    },
  },
  {
    position: { x: 800, y: -80, z: 300, direction: 0 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Pokemon Top Trumps",
    textSize: 10,
    plane: {
      width: 150,
      height: 50,
    },
  },
  {
    position: { x: 750, y: -80, z: 400, direction: Math.PI / -2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "GitHub",
    textSize: 10,
    plane: {
      width: 100,
      height: 50,
    },
  },
  {
    position: { x: 640, y: -80, z: 400, direction: Math.PI / -2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "LinkedIn",
    textSize: 10,
    plane: {
      width: 100,
      height: 50,
    },
  },
  {
    position: { x: 530, y: -80, z: 400, direction: Math.PI / -2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "CV",
    textSize: 10,
    plane: {
      width: 100,
      height: 50,
    },
  },
  {
    position: { x: 420, y: -80, z: 400, direction: Math.PI / -2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Creative Coding",
    textSize: 10,
    plane: {
      width: 100,
      height: 50,
    },
  },
  {
    position: { x: 300, y: -130, z: -300, direction: Math.PI / 2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Hi, I'm Melissa",
    textSize: 30,
    plane: {
      width: 250,
      height: 50,
    },
  },
  {
    position: { x: 300, y: -80, z: -300, direction: Math.PI / 2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "I'm a fullstack engineer using the PERN stack",
    textSize: 15,
    plane: {
      width: 250,
      height: 50,
    },
  },
  {
    position: { x: -100, y: -130, z: -500, direction: Math.PI / 2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Tech interests:",
    textSize: 30,
    plane: {
      width: 250,
      height: 50,
    },
  },
  {
    position: { x: -100, y: -80, z: -500, direction: Math.PI / 2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "WebDev | 3D graphics | AI/ML",
    textSize: 15,
    plane: {
      width: 250,
      height: 50,
    },
  },
  {
    position: { x: 300, y: -130, z: -700, direction: Math.PI / 2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Personal interests",
    textSize: 30,
    plane: {
      width: 250,
      height: 50,
    },
  },
  {
    position: { x: 300, y: -80, z: -700, direction: Math.PI / 2 },
    color: { v1: 0, v2: 0, v3: 0 },
    message: "Kpop | dance | art",
    textSize: 15,
    plane: {
      width: 250,
      height: 50,
    },
  },
];
